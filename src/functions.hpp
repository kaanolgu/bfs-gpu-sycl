    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (arg.find("--") == 0) {
        ...
        // Handle "--name value" format
          ...
          std::cerr << "Warning: No value provided for argument " << arg
                    << std::endl;
        }
      }
    }
...
std::vector<double> removeAnomalies(const std::vector<double>& data,
                                    double threshold) {
...
void printRow(const std::string& label, const std::string& value) {
  std::cout << "- " << std::left << std::setw(nameWidth)
            << std::setfill(separator) << label << " " << std::setw(numWidth)
            << std::setfill(separator) << value << "\n";
}
...
bool areAllRowsEqualToLast(const std::vector<std::vector<int>>& h_distancesGPU,
  ...
  for (size_t i = 0; i < h_distancesGPU.size() - 1; ++i) {
    ...
    if (!std::equal(h_distancesGPU[i].begin(), h_distancesGPU[i].end(),
                    lastRow.begin())) {
      std::cout << "\n distanceGPU[" << i
                << "] does not match the last distance \n";
      for (int m = 0; m < size; m++) {
        int countA =
            std::count(h_distancesGPU[i].begin(), h_distancesGPU[i].end(), m);
        std::cout << std::to_string(countA) << ", ";
      }
      for (size_t j = 0; j < h_distancesGPU[i].size(); ++j) {
        if (h_distancesGPU[i][j] != lastRow[j]) {
          std::cout << "Mismatch at index " << j << ": h_distancesGPU[" << i
                    << "][" << j << "] = " << h_distancesGPU[i][j]
                    << ", lastRow[" << j << "] = " << lastRow[j] << std::endl;
          int countA =
              std::count(h_distancesGPU[i].begin(), h_distancesGPU[i].end(),
                         h_distancesGPU[i][j]);
          int countB = std::count(lastRow.begin(), lastRow.end(), j);
          std::cout << "Nodes in level " << j << ": " << std::to_string(countA)
                    << " – " << std::to_string(countB) << std::endl;
        }
      }
...
struct DeviceInfo {
  ...
  long long
      edgesProcessed;  // Changed from totalEdgesProcessed to edgesProcessed
  double percentage;
};

void printDeviceInfo(const std::vector<DeviceInfo>& devices) {
  std::cout << std::left << std::setw(10) << "GPU ID" << std::setw(30)
            << "Edges Processed (*)" << std::setw(15) << "Percentage"
            << std::endl;

  std::cout << std::string(55, '-') << std::endl;  // Separator line

  for (const auto& device : devices) {
    std::cout << std::left << std::setw(10) << device.gpuID << std::setw(30)
              << device.edgesProcessed << std::setw(15) << std::fixed
              << std::setprecision(2) << device.percentage << " %" << std::endl;
  }

  // Add the note at the bottom
  std::cout << std::string(55, '-') << std::endl;  // Another separator line
  std::cout << "* : including already visited because we keep asking \"are you "
               "visited?\"]"
            << std::endl;
}