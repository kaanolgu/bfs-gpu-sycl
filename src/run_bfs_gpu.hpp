...
void printExecutionTimes(const std::vector<double> &execution_timesA,
                         const std::vector<double> &execution_timesB) {
  ...
  double grandTotal =
      std::accumulate(execution_timesA.begin(), execution_timesA.end(), 0.0) +
      std::accumulate(execution_timesB.begin(), execution_timesB.end(), 0.0);

  // Print header
  std::cout << std::left << std::setw(10) << "GPU ID" << std::setw(columnWidth)
            << "EN [ms]" << std::setw(percentageWidth) << "(%)"
            << std::setw(columnWidth) << "LG [ms]" << std::setw(percentageWidth)
            << "(%)" << std::setw(columnWidth) << "Total"
            << std::setw(percentageWidth) << "(%)" << std::endl;

  std::cout << std::string(10 + columnWidth * 3 + percentageWidth * 3, '-')
            << std::endl;  // Separator line
  ...
  for (int i = 0; i < NUM_TIMES; ++i) {
    ...
    std::cout << std::left << std::setw(10) << (i + 1) << std::setw(columnWidth)
              << std::fixed << std::setprecision(3) << execution_timesA[i]
              << std::setw(percentageWidth) << std::setprecision(2)
              << percentageA << "%" << std::setw(columnWidth) << std::fixed
              << std::setprecision(3) << execution_timesB[i]
              << std::setw(percentageWidth) << std::setprecision(2)
              << percentageB << "%" << std::setw(columnWidth) << std::fixed
              << std::setprecision(3) << rowTotal << std::setw(percentageWidth)
              << std::setprecision(2) << percentageRow << "%" << std::endl;
  }

  std::cout << std::string(10 + columnWidth * 3 + percentageWidth * 3, '-')
            << std::endl;  // Ending separator line
...
// Function to find the first and last non-zero indices in a vector, ignoring
// the specified start index
template <typename T>
std::pair<std::optional<size_t>, std::optional<size_t>> find_first_last_nonzero(
    const std::vector<T> &arr,
    size_t ignore_index = std::numeric_limits<size_t>::max()) {
...
void copyToDevice(queue &Q, std::vector<T> &arr, T *usm_arr,
  ...
  Q.memcpy(usm_arr, arr.data() + offset, (arr.size() - offset) * sizeof(T))
      .wait();
}
// initialize device arr with val, if needed set arr[pos] = pos_val
template <typename T>
...
#if USE_GLOBAL_LOAD_BALANCE == 1
template <int krnl_id>
void parallel_explorer_kernel(queue &q, const uint32_t V,
                              uint32_t *usm_nodes_start, uint32_t *usm_edges,
                              uint32_t *usm_pipe_1, uint32_t *usm_local_indptr,
                              uint8_t *usm_visit_mask, uint8_t *usm_visit,
                              uint32_t *usm_scan_temp, uint32_t *usm_scan_full,
                              const uint32_t Vstart, const uint32_t Vsize) {
  /*
  ...
  */
  ...
  const size_t local_size =
      q.get_device()
          .get_info<info::device::max_work_group_size>();  // Number of
                                                           // work-items per
                                                           // work-group
  ...
  auto e1 = q.submit([&](handler &h) {
    h.parallel_for<class ExploreNeighbours1<krnl_id>>(
        range1, [=](nd_item<1> item) {
          ...
          // 1. Read from usm_pipe, replace entry with corresponding edge
          // pointer
          if (gid < V) {
            uint32_t v = usm_pipe_1[gid];
            usm_local_indptr[gid] =
                usm_nodes_start[v];  // Replacing pipe entry with edge pointer
            local_node_deg = usm_nodes_start[v + 1] -
                             usm_nodes_start[v];  // Calculate each nodes
                                                  // partition specific degree
          } else {
            local_node_deg = 0;
          }
          ...
          uint32_t total_nnz = reduce_over_group(item.get_group(),
          local_node_deg, sycl::plus<>());
          sycl::group_barrier(item.get_group());
          ...
          // 3. Locally calculate inclusive sum, but store to global array
          shifted
          // by one to obtain exclusive sum of degrees to find total work items
          per block. usm_scan_full[gid+1] =
          sycl::inclusive_scan_over_group(item.get_group(), local_node_deg,
          sycl::plus<>());
          */

          // New variant using last result from scan also for block sum
          uint32_t running_sum = sycl::inclusive_scan_over_group(
              item.get_group(), local_node_deg, sycl::plus<>());
          sycl::group_barrier(item.get_group());
          if (gid < V + blockDim) {
  ...
  q.submit([&](handler &h) {
    h.parallel_for<class ExploreNeighbours2<krnl_id>>(
        range1, [=](nd_item<1> item) {
          const uint32_t blockDim = item.get_local_range(0);  // blockDim.x
          // single work group to calculate exclusive scan of block sum over all
          // blocks
          // TODO: might want to spawn with larger work group
          // note: last pointer to algorithm can be out of bounds here, will not
          // be accessed
          if (item.get_group(0) == 0) {
            sycl::joint_exclusive_scan(
                item.get_group(), usm_scan_temp,
                usm_scan_temp + (V + blockDim - 1) / blockDim + 1,
                usm_scan_temp, sycl::plus<>());
          }
        });
  });
#if VERBOSE
  ...
  q.submit([&](handler &h) {
    h.parallel_for<class ExploreNeighbours3<krnl_id>>(
        range1, [=](nd_item<1> item) {
          const uint32_t gid = item.get_global_id(0);         // global id
          const uint32_t blockDim = item.get_local_range(0);  // blockDim.x
          ...
          if (gid < V) {
            usm_scan_full[gid + 1] =
                usm_scan_full[gid + 1] + usm_scan_temp[item.get_group(0)];
          } /* else if(gid == V) {
            usm_scan_full[0] = 0;
          } else {
  ...
  q.submit([&](handler &h) {
    h.parallel_for<class ExploreNeighbours4<krnl_id>>(
        range4, [=](nd_item<1> item) {
          const uint32_t gid = item.get_global_id(0);  // global id
          const uint32_t lid = item.get_local_id(0);   // threadIdx.x
          // const uint32_t blockDim  = item.get_local_range(0); // blockDim.x
          ...
          // 4. Using grid stride loop with binary search to find the
          // corresponding edges
          for (int i = gid;       // global idx
            ...
            // uint32_t start = std::upper_bound(usm_scan_full, usm_scan_full+V,
            // i) - usm_scan_full; manually inlined
            uint32_t start = 0;
...
void parallel_explorer_kernel(queue &q, const uint32_t V,
                              uint32_t *usm_nodes_start, uint32_t *usm_edges,
                              uint32_t *usm_pipe_1, uint8_t *usm_visit_mask,
                              uint8_t *usm_visit, const uint32_t Vstart,
  ...
  q.submit([&](handler &h) {
    ...
    h.parallel_for<class ExploreNeighbours<krnl_id>>(
        range, [=](nd_item<1> item) {
#if USE_STRIDED_LOCAL_LOAD_BALANCE
          const int gid =
              item.get_group(0) + item.get_local_id(0) * V_stride;  // global id
#else
          ...
          if (gid < V) {
            v = usm_pipe_1[gid];
            edge_pointer[lid] =
                usm_nodes_start[v];  // Store pointer to the edges start
            local_node_deg = usm_nodes_start[v + 1] -
                             usm_nodes_start[v];  // Calculate each nodes
                                                  // partition specific degree
          } else {
          ...
          // 2. Cumulative sum of total number of nonzeros
          uint32_t total_nnz = reduce_over_group(
              item.get_group(), local_node_deg, sycl::plus<>());
          /*  Commenting out length check because it generates mismatching
             results in between the executions like previously with the
             group_barrier missing issue we had. Possibly group_barrier does not
             block gid and it keeps counting */
          // uint32_t length = (V < gid - lid + blockDim) ? (V - (gid -lid)) :
          // blockDim;
          // 3. Exclusive sum of degrees to find total work items per block.
          degrees[lid] = sycl::exclusive_scan_over_group(
              item.get_group(), local_node_deg, sycl::plus<>());
          // the exclusive scan over group does not sync the results so
          // group_barrier is needed here
          sycl::group_barrier(item.get_group());
...
template <int krnl_id>
void parallel_levelgen_kernel(queue &q, const uint32_t Vstart,
                              const uint32_t Vsize, uint8_t *usm_visit_mask,
                              uint8_t *usm_visit, const int level_plus_one,
                              uint32_t *usm_pipe, uint32_t *usm_pipe_size,
                              int *usm_dist) {
  // Define the work-group size and the number of work-groups
  const size_t local_size =
      q.get_device()
  ...
  const size_t global_size =
      ((Vsize + local_size - 1) / local_size) * local_size;
  const size_t V_stride = (Vsize + local_size - 1) / local_size;

  // Setup the range
  nd_range<1> range(global_size, local_size);
  q.submit([&](handler &h) {
    h.parallel_for<class LevelGen<krnl_id>>(
        range, [=](nd_item<1> item) [[intel::kernel_args_restrict]] {
    // const int gid = item.get_global_id(0);    // global id

#if USE_STRIDED_LOCAL_LOAD_BALANCE
          const int gid = item.get_group(0) +
                          item.get_local_id(0) * V_stride;  // strided global id
#else
            const int gid = item.get_global_id(0);    // global id
#endif
          if (gid < Vsize) {
...
void GPURun(uint32_t vertexCount, vectorT &IndexHost, vectorT &OffsetHost,
            std::vector<uint8_t> &h_visit_mask, std::vector<uint8_t> &h_visit,
            std::vector<std::vector<int>> &h_distance, int sourceNode,
            const int num_runs, nlohmann::json &newJsonObj,
            std::vector<uint32_t> &h_visit_offsets) noexcept(false) {
  try {
    ...
    std::transform(Devs.begin(), Devs.begin() + NUM_GPU,
                   std::back_inserter(Queues), [](const sycl::device &D) {
                     return sycl::queue{
                         D,
    ...
    std::cout
        << "SM_COUNT        : "
        << Queues[0].get_device().get_info<info::device::max_compute_units>()
        << std::endl;
    std::cout << "SM_FACTOR       : " << SM_FACTOR << std::endl;
    std::cout
        << "LOCAL_WORK_SIZE : "
        << Queues[0].get_device().get_info<info::device::max_work_group_size>()
        << std::endl;
    ...
    for (int i = 0; i < Queues.size(); i++) {
      std::cout << i << ":\t"
                << Queues[i].get_device().get_info<sycl::info::device::name>()
                << std::endl;
    }
    std::cout << "----------------------------------------" << std::endl;
    ...
    for (int i = 0; i < num_runs; i++) {
      ...
      uint32_t *usm_pipe_global =
          malloc_device<uint32_t>(vertexCount, Queues[0]);
      uint32_t *usm_pipe_global_ =
          malloc_device<uint32_t>(vertexCount, Queues[0]);
      uint32_t *usm_pipe_read_pointer =
          usm_pipe_global;  // Start with the first buffer
      uint32_t *usm_pipe_write_pointer =
          usm_pipe_global_;  // Start with the second buffer
      ...
      gpu_tools::UnrolledLoop<NUM_GPU>([&](auto gpuID) {
        ...
        OffsetDevice[gpuID] =
            malloc_device<uint32_t>(offsetSize, Queues[gpuID]);
        EdgesDevice[gpuID] = malloc_device<uint32_t>(indexSize, Queues[gpuID]);
        usm_visit_mask[gpuID] = malloc_device<uint8_t>(
            (h_visit_offsets[gpuID + 1] - h_visit_offsets[gpuID]),
            Queues[gpuID]);
        usm_visit[gpuID] = malloc_device<uint8_t>(
            (h_visit_offsets[gpuID + 1] - h_visit_offsets[gpuID]),
            Queues[gpuID]);
#if USE_GLOBAL_LOAD_BALANCE == 1

        const uint32_t scanTempSize =
            (offsetSize - 1 +
             Queues[0]
                 .get_device()
            ...
            Queues[0]
                .get_device()
                .get_info<info::device::max_work_group_size>();
        usm_local_indptr[gpuID] =
            malloc_device<uint32_t>((offsetSize), Queues[gpuID]);
        const uint32_t scanFullSize = offsetSize;  // offsetSize;
        ScanTempDevice[gpuID] =
            malloc_device<uint32_t>(scanTempSize, Queues[gpuID]);
        ScanFullDevice[gpuID] =
            malloc_device<uint32_t>(scanFullSize, Queues[gpuID]);
        if (i == 0) {
          std::cout << "Allocated " << scanTempSize
                    << " elements for ScanTempDevice buffer." << std::endl;
          std::cout << "Allocated " << scanFullSize
                    << " elements for ScanFullDevice buffer." << std::endl;
        }
        ...
        */
        Queues[gpuID]
            .memset(ScanFullDevice[gpuID], 0, 1 * sizeof(uint32_t))
            .wait();
        ...
        Queues[0].memcpy(usm_visit_mask[gpuID],
                         h_visit_mask.data() + h_visit_offsets[gpuID],
                         (h_visit_offsets[gpuID + 1] - h_visit_offsets[gpuID]) *
                             sizeof(uint8_t));
        Queues[0].memcpy(usm_visit[gpuID],
                         h_visit.data() + h_visit_offsets[gpuID],
                         (h_visit_offsets[gpuID + 1] - h_visit_offsets[gpuID]) *
                             sizeof(uint8_t));
      });
      ...
      for (int iteration = 0; iteration < MAX_NUM_LEVELS; iteration++) {
        ...
        gpu_tools::UnrolledLoop<NUM_GPU>([&](auto gpuID) {
#if USE_GLOBAL_LOAD_BALANCE == 1
          parallel_explorer_kernel<gpuID>(
              Queues[gpuID], h_pipe_count[0], OffsetDevice[gpuID],
              EdgesDevice[gpuID], usm_pipe_read_pointer,
              usm_local_indptr[gpuID], usm_visit_mask[gpuID], usm_visit[gpuID],
              ScanTempDevice[gpuID], ScanFullDevice[gpuID],
              h_visit_offsets[gpuID],
              h_visit_offsets[gpuID + 1] - h_visit_offsets[gpuID]);
#else
          parallel_explorer_kernel<gpuID>(
              Queues[gpuID], h_pipe_count[0], OffsetDevice[gpuID],
              EdgesDevice[gpuID], usm_pipe_read_pointer, usm_visit_mask[gpuID],
              usm_visit[gpuID], h_visit_offsets[gpuID],
              h_visit_offsets[gpuID + 1] - h_visit_offsets[gpuID]);
#endif
          parallel_levelgen_kernel<gpuID>(
              Queues[gpuID], h_visit_offsets[gpuID],
              h_visit_offsets[gpuID + 1] - h_visit_offsets[gpuID],
              usm_visit_mask[gpuID], usm_visit[gpuID], iteration + 1,
              usm_pipe_write_pointer, frontierCountDevice, usm_dist);
        });
        ...
        */
        gpu_tools::UnrolledLoop<NUM_GPU>(
            [&](auto gpuID) { Queues[gpuID].wait(); });
...
// Capture execution times
        ...
        gpu_tools::UnrolledLoop<NUM_GPU>([&](auto gpuID) {
          ...
          if (i == 0)
            std::cout << std::setw(13) << h_pipe_count[0]
                      << " new nodes after iteration " << std::setw(2)
                      << iteration << std::endl;
        });
#endif
      }
    ...
    double threshold = 1.5;  // This means we consider points beyond 1.5
                             // standard deviations as anomalies
    // https://bookdown.org/kevin_davisross/probsim-book/normal-distributions.html
    // 87% explanation for threshold :
    // https://chatgpt.com/share/6e64d349-bdd6-4662-99c2-2d265dffd43c Remove
    // anomalies
    ...
    double total_exec_time =
        std::accumulate(run_times.begin(), run_times.end(), 0.0);
    double avg_time = std::accumulate(run_times.begin(), run_times.end(), 0.0) /
                      run_times.size();
    double total_time_filtered =
        std::accumulate(filteredData.begin(), filteredData.end(), 0.0) /
        filteredData.size();
    double minimum_time_filtered =
        *std::min_element(filteredData.begin(), filteredData.end());
    double total_time_90f =
        std::accumulate(filteredDataNOF.begin(), filteredDataNOF.end(), 0.0) /
        filteredDataNOF.size();
    double minimum_time_90f =
        *std::min_element(filteredDataNOF.begin(), filteredDataNOF.end());

    // Print events and execution times
    printRow("Average (filtered) Time:",
             formatDouble(total_time_filtered) + " (ms)");
    printRow("Minimum (filtered) Time:",
             formatDouble(minimum_time_filtered) + " (ms)");
