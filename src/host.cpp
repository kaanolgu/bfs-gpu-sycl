...
bool print_levels(std::vector<datatypeA>& A, std::string nameA,
                  std::vector<std::vector<datatypeB>>& B, std::string nameB,
                  int size) {
  ...
  bool status = std::equal(A.begin(), A.end(), B.back().begin()) &&
                areAllRowsEqualToLast(B, size);
...
int main(int argc, char* argv[]) {
  ...
  std::cout << std::setw(20) << std::left << "- num_gpu" << std::setw(20)
            << NUM_GPU << std::endl;
  ...
  std::cout << std::setw(20) << std::left << "- # vertices" << std::setw(20)
            << numCols << std::endl;
  std::cout << std::setw(20) << std::left << "- # edges" << std::setw(20)
            << numEdges << std::endl;
  std::cout << std::setw(20) << std::left << "- dataset" << std::setw(20)
            << "Load [OK]" << std::endl;
#if USE_GLOBAL_LOAD_BALANCE == 1
  std::cout << std::setw(20) << std::left << "- load balance" << std::setw(20)
            << "[GLOBAL]" << std::endl;
#else
#if USE_STRIDED_LOCAL_LOAD_BALANCE
  std::cout << std::setw(20) << std::left << "- load balance" << std::setw(20)
            << "[STRIDED-LOCAL]" << std::endl;
#else
  std::cout << std::setw(20) << std::left << "- load balance" << std::setw(20)
            << "[LOCAL]" << std::endl;
#endif
  ...
  for (uint32_t i = 0; i < h_visit_offsets.size(); i++)
    std::cout << "- GPU[" << std::to_string(i) << "] OFFSET:\t"
              << h_visit_offsets[i] << std::endl;
  std::cout << "----------------------------------------" << std::endl;
#endif

  if (NUM_GPU > 1) {
    GPURun(numRows, graph.indsMulti, graph.indptrMulti, h_updating_graph_mask,
           h_graph_visited, h_distancesGPU, start_vertex, num_runs, newJsonObj,
           h_visit_offsets);
  } else {
    GPURun(numRows, graph.inds, graph.indptr, h_updating_graph_mask,
           h_graph_visited, h_distancesGPU, start_vertex, num_runs, newJsonObj,
           h_visit_offsets);
  }
...
if (!hasCachedResult) {
  ...
  // check
  auto it =
      std::max_element(h_distancesGPU[0].begin(), h_distancesGPU[0].end());
  // Check if iterator is not pointing to the end of vector
  int maxLevelCPU = (*it + 2);
  std::cout << "----------------------------------------" << std::endl;
  bool status = print_levels(host_level, "cpu", h_distancesGPU, "fpga",
                             maxLevelCPU);  // CPU Results
  newJsonObj["valid"] = status;
  std::cout << "\n----------------------------------------" << std::endl;
  std::cout << (status ? "[SUCCESS]" : "[FAILURE]") << "\tAll results on GPUs ("
            << num_runs << " runs) cross checked vs CPU results match\n";
  std::cout << "----------------------------------------" << std::endl;
  ...
  newJsonObj["avgMTEPS"] =
      (static_cast<unsigned int>(newJsonObj["edgesCount"]) /
       (1000000 * static_cast<double>(newJsonObj["avgExecutionTime"]) * 1e-3));
  newJsonObj["avgMTEPS90f"] =
      (static_cast<unsigned int>(newJsonObj["edgesCount"]) /
       (1000000 * static_cast<double>(newJsonObj["avgExecutionTime90f"]) *
        1e-3));
  double gtepsValue =
      static_cast<unsigned int>(newJsonObj["edgesCount"]) /
      (1000000000 * static_cast<double>(newJsonObj["avgExecutionTime90f"]) *
       1e-3);
  std::ostringstream streamObj;
  streamObj << std::fixed << std::setprecision(2) << gtepsValue;
  newJsonObj["avgGTEPS90f"] = streamObj.str();

  newJsonObj["avgMTEPSFilter"] =
      (static_cast<unsigned int>(newJsonObj["edgesCount"]) /
       (1000000 * static_cast<double>(newJsonObj["avgExecutionTimeFiltered"]) *
        1e-3));
  newJsonObj["maxMTEPSFilter"] =
      (static_cast<unsigned int>(newJsonObj["edgesCount"]) /
       (1000000 * static_cast<double>(newJsonObj["minExecutionTimeFiltered"]) *
        1e-3));
  newJsonObj["edgesCoverage"] =
      static_cast<uint64_t>(newJsonObj["edgesCount"]) / numEdges * 100.0;
  // Add counts from the second dimension (A.back()) directly into "Levels" key
  for (int i = 0; i < maxLevelCPU - 1; i++) {
    int countA = std::count(h_distancesGPU.back().begin(),
                            h_distancesGPU.back().end(), i);
  ...
  if (inFile.is_open()) {
    try {
      ...
      std::cerr << "Error reading or parsing existing JSON data: " << e.what()
                << std::endl;
  ...
  std::string datasetKey =
      datasetName + "_" + std::to_string(num_runs) + "_" + "global";
#else
#if USE_STRIDED_LOCAL_LOAD_BALANCE
  std::string datasetKey =
      datasetName + "_" + std::to_string(num_runs) + "_" + "local_strided";
#else
  std::string datasetKey =
      datasetName + "_" + std::to_string(num_runs) + "_" + "local";
#endif
#endif
  ...
  if (outFile.is_open()) {
    outFile << combinedJsonObj.dump(
        4);  // Pretty-print with 4-space indentation
    outFile.close();
  } else {
