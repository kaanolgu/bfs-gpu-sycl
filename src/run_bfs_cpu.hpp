...
void saveCpuReference(const std::string &h5path,
                      const std::vector<int> &host_level, int start_vertex) {
  ...
  H5::DataSet ds =
      refGroup.createDataSet(dsName, H5::PredType::NATIVE_INT32, space);
  ds.write(host_level.data(), H5::PredType::NATIVE_INT32);
}
bool loadCpuReference(const std::string &h5path, std::vector<int> &host_level,
                      int start_vertex) {
